...
for file in input_files:
    with open(f'{TEST_DIR}/{file}', 'r') as inp_file:
        with open(f'{PROGRAM_OUTPUT_DIR}/{file.removesuffix(".in")}.pgo', 'w') as program_output:
            subprocess.run(['java', '-cp', '.', 'Main'], stdin=inp_file, stdout=program_output)

number_passed = 0
for num, file in enumerate(expected_out):
    with open(f'{PROGRAM_OUTPUT_DIR}/cmp/{file.removesuffix(".out")}.cmp', 'w') as cmp:
